namespace BackendAPI.Services.Storage
{
    public interface IStorageService
    {
        ...
        Task SaveCanvasJsonAsync(int storyId, int userId, string json);

        // 定義獲取和保存用戶角色的方法
        Task<string?> GetCharacterJsonAsync(int storyId, int userId);
        Task SaveCharacterJsonAsync(int storyId, int userId, string json);

        // 定義獲取和保存用戶時間線的方法
        Task<string?> GetTimelineJsonAsync(int storyId, int userId);
        Task SaveTimelineJsonAsync(int storyId, int userId, string json);
    }
}
